...
public class ArrayOperation {
     ...
     * 堆排序
     * 建堆
     * 堆结点跟堆的最后一个叶子结点交换，然后剩下的部分又调整，只用调整堆顶元素即可（出队逻辑）
     *
     * 数组的下标对应着树从上往下层次遍历的序号
     *
     * 堆插入：在数组末尾添加元素，然后该元素跟其父元素比较，若不满足要求，则交换，像这样一直往上，直到满足条件或者到达根结点
     * 不用担心交换后，该元素是否跟原来的父结点的另一个孩子不满足要求
     * 以最大堆为例，如果要交换，说明当前结点大于父结点，而父结点是大于另一个孩子结点的，自然当前结点也大于另一个孩子结点
     */
