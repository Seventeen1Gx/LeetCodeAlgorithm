...
public class TrappingRainWater {
    ...
    public int solution3_official(int[] height) {
        ...
        while (current < height.length) {
            ...
            // 说明此时 current 是积水右边的那堵墙，然后往左遍历，一直到左墙壁
    ...
    // 启发双指针法，根据上面第二种方法改进
    // 首先方法二中求得 maxLeft 数组和 maxRight 数组后
    // 在计算积水时，每次循环其实只用对应位置的元素，而之后不再使用，所以我们可以节约数组的空间
    // 但下面只省了 maxLeft 数组，是因为我们的循环是从左到右的，可以用 maxLeft 变量保存当前遇到的最高柱子高度
    ...
    // 双指针法，有了上面这个方法的启发，之所以不能省略 maxRight 数组的空间，是因为我们只有一个指针 i，其遍历为从左往右
    // 故我们需要两个指针，left 从左向右遍历，right 从右往左遍历，直到两针相遇，也就遍历了所有列(还是按列计算)
    // 我们用 maxLeft 来表示 left 左侧(不包括 left )最大高度，maxRight 表示 right 右侧(不包括 right )最大高度
    ...
    public int solution4_official(int[] height) {
        ...
        // 每次循环计算 left 所指列或 right 所指列上的积水
        while (left < right) {
            if (height[left] < height[right]) {
                //计算 left 列的积水，因为 maxleft 可以确定水平面。为什么呢？
